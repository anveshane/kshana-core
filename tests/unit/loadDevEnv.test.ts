...
describe("loadDevEnv", () => {
  ...
  it("returns projectsDir = KSHANA_PROJECTS_DIR override when set", () => {
    const original = process.env["KSHANA_PROJECTS_DIR"];
    process.env["KSHANA_PROJECTS_DIR"] = "/some/explicit/projects/path";
    try {
      const result = loadDevEnv(fakeRoot);
      expect(result.projectsDir).toBe("/some/explicit/projects/path");
    } finally {
      if (original === undefined) delete process.env["KSHANA_PROJECTS_DIR"];
      else process.env["KSHANA_PROJECTS_DIR"] = original;
    }
  });

  it("returns projectsDir = ~/Kshana when KSHANA_PACKAGED=1 (packaged desktop)", () => {
    ...
    try {
      const result = loadDevEnv(fakeRoot);
      // Should end with /Kshana (homedir prefix varies per machine).
      expect(result.projectsDir).toMatch(/\/Kshana$/);
    } finally {
      if (originalPkg === undefined) delete process.env["KSHANA_PACKAGED"];
      else process.env["KSHANA_PACKAGED"] = originalPkg;
