 ...
 * users may not have ffmpeg at all. We expose KSHANA_FFMPEG_PATH and
 * KSHANA_FFPROBE_PATH so the desktop wrapper can point kshana-core
 * at its bundled @ffmpeg-installer / @ffprobe-installer binaries.
 */
describe('ffmpeg/ffprobe binary path resolver', () => {
  const prevFfmpeg = process.env.KSHANA_FFMPEG_PATH;
  const prevFfprobe = process.env.KSHANA_FFPROBE_PATH;

  beforeEach(() => {
    delete process.env.KSHANA_FFMPEG_PATH;
    delete process.env.KSHANA_FFPROBE_PATH;
  });

  afterEach(() => {
    if (prevFfmpeg === undefined) delete process.env.KSHANA_FFMPEG_PATH;
    else process.env.KSHANA_FFMPEG_PATH = prevFfmpeg;
    if (prevFfprobe === undefined) delete process.env.KSHANA_FFPROBE_PATH;
    else process.env.KSHANA_FFPROBE_PATH = prevFfprobe;
  });
  ...
  it('honors KSHANA_FFMPEG_PATH and KSHANA_FFPROBE_PATH overrides', () => {
    process.env.KSHANA_FFMPEG_PATH = '/opt/bundled/ffmpeg';
    process.env.KSHANA_FFPROBE_PATH = '/opt/bundled/ffprobe';
  ...
  it('reads env vars at call time (not at module load)', () => {
    process.env.KSHANA_FFMPEG_PATH = '/first/ffmpeg';
    expect(getFfmpegPath()).toBe('/first/ffmpeg');
    process.env.KSHANA_FFMPEG_PATH = '/second/ffmpeg';
    expect(getFfmpegPath()).toBe('/second/ffmpeg');
  });

  it('runFFmpeg spawns the binary at KSHANA_FFMPEG_PATH', async () => {
    ...
    const sentinel = '/nonexistent/kshana-test-ffmpeg-sentinel';
    process.env.KSHANA_FFMPEG_PATH = sentinel;
