/**
 * Regression: when kshana-ink is bundled (e.g. for the embedded
...
describe("findKshanaInkRoot", () => {
  it("walks up from a bundled file (dist/server/manager.js) to the kshana-ink root", () => {
    const fakeRoot = mkdtempSync(join(tmpdir(), "kshana-ink-root-"));
    writeFileSync(
    ...
    expect(findKshanaInkRoot(pathToFileURL(bundlePath).href)).toBe(
      resolve(fakeRoot),
    );
  });

  it("walks up from a source file (src/agent/pi/paths.ts) to the kshana-ink root", () => {
    const fakeRoot = mkdtempSync(join(tmpdir(), "kshana-ink-root-"));
    writeFileSync(
      join(fakeRoot, "package.json"),
      JSON.stringify({ name: "kshana-ink", version: "0.0.0" }),
    ...
    expect(findKshanaInkRoot(pathToFileURL(srcPath).href)).toBe(
      resolve(fakeRoot),
    );
  });

  it("ignores intermediate package.json files that aren't kshana-ink", () => {
    // The desktop's release/app/node_modules/kshana-ink case: walking
    // up from the bundle hits node_modules' own package.jsons (other
    // packages) before reaching kshana-ink's. The walker must keep
    ...
    expect(findKshanaInkRoot(pathToFileURL(fakeFileInDep).href)).toBe(
      resolve(fakeRoot),
    );
  });
