/**
 * Autoresearch: iteratively optimize the character_image guide prompt
 * using binary judge feedback.
 *
 * Loop:
 *   1. Generate character_image prompt (3 runs for stability)
 ...
 *
 * Usage: npx tsx test-output/autoresearch-character-ref.ts
 */
...
// PROJECT_DIR used as fallback for buildSourceContext when needed
const RUBRIC_PATH = path.join(PROJECT_ROOT, 'tests', 'autoresearch', 'rubrics', 'character-image-binary.json');
const GUIDE_PATH = path.join(PROJECT_ROOT, 'prompts', 'skills', 'defaults', 'character_image_guide.md');
const SKILL_PATH = path.join(PROJECT_ROOT, 'prompts', 'skills', 'content-type', 'character_image_prompt.comfyui.zimage.md');
const CLAUDE_MODEL = process.env['EVAL_CLAUDE_MODEL'] ?? 'sonnet';
const RESULTS_DIR = path.join(PROJECT_ROOT, 'test-output', 'autoresearch-results');
...
const TEST_CHARACTERS: Array<{
  ...
  { label: 'Warden (alien predator)', project: 'humanitys_cycle_long_humanity-2.kshana', name: 'warden' },
  { label: 'Keerti (Indian teenager)', project: 'keerti_extremely_beautiful_young.kshana', name: 'keerti' },
  { label: 'Kaito (Japanese baker)', project: 'quiet_morning_baker_sat-2.kshana', name: 'kaito' },
  { label: 'Maya/The Runner (action)', project: 'story_begins_girl_sprinting.kshana', name: 'the_runner' },
  { label: 'Young Girl (fantasy child)', project: 'centuries_ago_continent_elarion.kshana', name: 'young_girl' },
];
...
async function generateAndJudge(
  ...
  tc: typeof TEST_CHARACTERS[number],
...
## Context
The guide prompt below instructs a local LLM to write character image prompts.
The guide must be MODEL-AGNOSTIC — it will be used with different LLMs (Qwen, Llama, etc.).
The LLM reads a character profile and the guide, then outputs an image generation prompt.
A binary judge evaluates the output against ${rubric.questions.length} yes/no quality questions.

This iteration tested ${failures.totalRuns} DIVERSE characters (alien, teenager, elderly, action hero, child).
...
- The character profile content is injected in the USER prompt after the guide. The guide must tell the LLM to EXTRACT details from the profile.
...
- Handle non-human characters: if the character has no hair, say "no hair, [describe head covering]". If no clothing, say "no clothing, bare [skin description]".
- ALWAYS require: plain neutral background (never environments), simple even studio lighting (never dramatic/cinematic).
- The guide is tested across DIVERSE characters (alien predators, human teens, elderly, action heroes, fantasy children). Make sure instructions handle all these cases.
...
async function main() {
  ...
  console.log(`Autoresearch: Character Image Guide Optimization (Multi-Character)`);
  console.log(`LLM: ${config.model} | Judge: claude --model ${CLAUDE_MODEL}`);
  console.log(`Characters: ${TEST_CHARACTERS.map(tc => tc.label).join(', ')}`);
  console.log(`Runs/iter: ${TEST_CHARACTERS.length} (1 per character) | Max iters: ${MAX_ITERATIONS}`);
  ...
  for (let iter = 1; iter <= MAX_ITERATIONS; iter++) {
    ...
    // 1. Run one generation + judge per character
    ...
    for (let i = 0; i < TEST_CHARACTERS.length; i++) {
      const tc = TEST_CHARACTERS[i]!;
      console.log(`  [${i + 1}/${TEST_CHARACTERS.length}] ${tc.label}...`);
