...
**Count the reference images listed as available for this shot. Use only those numbers.** If two images are listed, reference only "image 1" and "image 2." Never reference "image 3" if only two images are available. Never fabricate references not explicitly provided.
...
Reference images by their number in the prose: "the character from image 1," "the setting from image 2." Unreferenced images are ignored by the model.
